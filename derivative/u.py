...
def darw_line2(dot1:Dot,dot2:Dot,**kwargs)->Line:
    '''连线'''
    # if(kwargs):
        # if(kwargs["c"]): c=kwargs["c"]
    return Line(np.array((dot1.get_x(),dot1.get_y(),0)),np.array((dot2.get_x(),dot2.get_y(),0)),color=WHITE)

def darw_t_line(axes:Axes,x1:2,func,length:10,**kwargs)->Line:
    '''该点切线'''
    c=WHITE
